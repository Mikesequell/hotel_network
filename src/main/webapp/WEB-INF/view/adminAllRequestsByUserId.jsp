...
<section class="section">
    <table>
        ...
        <tbody>
        <c:forEach items="${allRequestsByUserId}" var="request">
            <tr>
                <td>${request.id}</td>
...
<%--    <c:forEach items="${allRequestsByUserId}" var="request">--%>
<%--&lt;%&ndash;    <a href="<c:url value="/admin/showAllRequestsByUser?id=${user.id}"/>">${hotel.username}</a> <br>&ndash;%&gt;--%>
<%--    <a>${request.id} -- username: ${request.userId.username} -- status: ${request.statusId.name} -- arrival date: ${request.arrivalDate} -- departure date: ${request.departureDate}</a> <br>--%>
<%--    </c:forEach>--%>
</body>
</html>